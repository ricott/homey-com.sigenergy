...
class Base extends HomeyEventEmitter {
     ...
     *   - If a coalesced range read fails for any other reason (e.g. the device
     *     rejects a span crossing an unmapped register), that run falls back to
     *     per-register reads and is blocklisted so later polls skip the range
     *     attempt. Individual register failures are still tolerated (slot =
     *     null, flagged until it recovers).
     *
        ...
        for (const run of runs) {
            ...
            if (run.registers.length === 1 || this.#coalesceBlocklist.has(runKey)) {
                for (const reg of run.registers) {
                    buffers[reg.key] = await this.#readSingleRegister(reg, client, label);
                }
                continue;
            }
     ...
     * recovers. A connection-level error (see #shouldRebuildModbus) is rethrown.
     */
    async #readSingleRegister(registry, client, label) {
        try {
